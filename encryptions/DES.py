...
# Left rotations for key schedule
...
def s_box_substitution(bits):
    ...
    for i in range(8):
        ...
        result.extend([int(b) for b in f"{value:04b}"])
    return result
...
def generate_keys(key):
    ...
    for rotation in LEFT_ROTATIONS:
        left = rotate_left(left, rotation)
        right = rotate_left(right, rotation)
...
def des_encrypt(block, keys):
    return des_block(block, keys, encrypt=True)

def des_decrypt(block, keys):
    return des_block(block, keys, encrypt=False)
...
def des_encrypt_message(plaintext, key):
    ...
    for i in range(0, len(plaintext_bits), 64):
        ...
        block = des_encrypt(block, keys)
        ciphertext_bits.extend(block)
    return bits_to_str(ciphertext_bits)

def des_decrypt_message(ciphertext, key):
    ciphertext_bits = str_to_bits(ciphertext)
    plaintext_bits = []
    keys = generate_keys(key)
    for i in range(0, len(ciphertext_bits), 64):
        block = ciphertext_bits[i:i + 64]
        block = des_decrypt(block, keys)
        plaintext_bits.extend(block)
    plaintext = bits_to_str(plaintext_bits)
...
if __name__ == "__main__":
    ...
    plaintext = "Hello, DES3!!!!!!!!!!"

    ciphertext = des_encrypt_message(plaintext, key)
    print(f"Ciphertext: {ciphertext}")
    decrypted_text = des_decrypt_message(bytes(ciphertext.encode()).decode(), key)
